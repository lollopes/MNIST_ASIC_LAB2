...
class BNN_SW(nn.Module):
    ...
    def __init__(self, in_features = 28*28, HL_1=100, HL_2=100, out_features=10,dropout = 0.5):
        ...
        self.fc3 = BinarizeLinear(HL_2, out_features, bias = False)
...
class BNN_HW(nn.Module):
    ...
    def __init__(self, in_features = 28*28, HL_1=100, HL_2=100, out_features=10):
...
search_space = []
import itertools
#search_space[   0      1    2  3    4       5   ]
#search_space[layer1,layer2,LR,DO,EPOCH_N,BATCH_N]
for r in itertools.product(HL_1,HL_2,LR,DO,EPOCH_N,BATCH_N): search_space.append((r[0],r[1],r[2],r[3],r[4],r[5]))
...
for space_idx, space in  enumerate(search_space):
    ...
    train_loader = DataLoader(train_data, batch_size=space[5], shuffle=True)
    test_loader = DataLoader(test_data)

    # Inner loop for epochs 
    for epoch in range(1, space[4] + 1):
        train(epoch,model_sw,space_idx,space[2])
    ...
    res.append(('H1:{} H2:{} LR:{} DO:{} EPOCH:{} BATCH:{}'.format(space[0],space[1],space[2],space[3],space[4],space[5]),accuracy_sw.item(),accuracy_hw.item()))
...
np.savetxt("optimize_28_28_hidden_1_res.csv", res, fmt = '%s,%f,%f', delimiter = ",")
...
plt.savefig("optimize_28_28_hidden_1_plt.jpg",bbox_inches='tight', dpi=150)
